# Microservicio Rutas — CRUD (FastAPI + PostgreSQL)

## Descripción General
Este microservicio, desarrollado con **Python 3.11** y **FastAPI**, implementa operaciones **CRUD** sobre programacion de rutas, incluyendo creación, lectura y validación de datos. El módulo principal `crud.py` encapsula la lógica de negocio, interactuando con SQLAlchemy y modelos Pydantic.
...
│
├─── tests/
│       test_proveedor.py       # Pruebas unitarias e integración
│
...
```bash
git clone https://github.com/tuusuario/rutas.git
cd rutas
```
...
```bash
DATABASE_URL=postgresql://postgres\:postgres@localhost:5432/rutas
...
| `get_rutas` | Devuelven listas ordenadas alfabéticamente. |
| `create_ruta` | Inserta un nuevo producto en la base de datos. |
...
El workflow ubicado en `.github/workflows/rutas-ci.yml`:
...
Microservicio: *Rutas (CRUD con FastAPI y PostgreSQL)*
